...
**Generated packages.** `packages/types` and `packages/client` are git-ignored and regenerated by `pnpm generate`; a fresh clone must run it before typecheck will pass. Never commit their output — a tracked copy is a second version of the contract that can drift. New generated directories go in `eslint.config.mjs`'s `ignores` with **package-specific** patterns; a broad `**/src/**` would silence real source. Strictness may be relaxed for generated code **only in that package's own tsconfig**, never in `tsconfig.base.json`.
